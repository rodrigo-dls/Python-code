...
# circle radius is 1
# square corners are (-1,-1), (1, -1), (1, 1), and (-1, 1)

# Declare variables
n = 0                                                       # the total number of points that fall inside circle A
N = int(input("Enter the number of points to generate: "))  # the total number of random points
i = 0
while i < N:
    coord_x = random.random() * 2 -1
    coord_y = random.random() * 2 -1
    coord_distance = coord_x**2 + coord_y**2

    if coord_distance < 1:                                  # check if the point is inside the unit circle
        n += 1
    i += 1
